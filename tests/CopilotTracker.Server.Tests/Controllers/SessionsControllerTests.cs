...
public class SessionsControllerTests
{
    ...
    public async Task List_Returns200WithPagedResults()
    {
        ...
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 50, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, null, null, 50);
    ...
    public async Task List_WithNonexistentMachineId_ReturnsEmptyResults()
    {
        ...
        _sessionService
            .Setup(s => s.ListAsync("nonexistent-machine", null, null, null, null, 50, null))
            .ReturnsAsync(empty);

        var result = await _controller.List("nonexistent-machine", null, null, null, null, null, 50);
    ...
    public async Task List_WithStatusFilter_PassesFilterToService()
    {
        ...
        _sessionService
            .Setup(s => s.ListAsync(null, "active", null, null, null, 50, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, "active", null, null, null, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, "active", null, null, null, 50, null), Times.Once);
    }
    ...
    public async Task List_WithSinceFilter_PassesDateToService()
    {
        ...
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, since, null, 50, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, since, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, since, null, 50, null), Times.Once);
    }
    ...
    public async Task List_WithContinuationToken_PassesTokenToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, "token123", 50, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, null, "token123", 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, null, "token123", 50, null), Times.Once);
    }
    ...
    public async Task List_WithCustomPageSize_PassesPageSizeToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 10, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, null, null, 10);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, null, null, 10, null), Times.Once);
    }
    ...
    public async Task List_WithLargePageSize_PassesValueThrough()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 10000, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, null, null, 10000);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, null, null, 10000, null), Times.Once);
    }
    ...
    public async Task List_WithMultiplePages_ReturnsContinuationToken()
    {
        ...
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 1, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, null, null, null, 1);
    ...
    public async Task List_ServiceThrows_ExceptionPropagates()
    {
        ...
        Func<Task> act = () => _controller.List(null, null, null, null, null, null, 50);
    ...
    public async Task List_AllFiltersApplied_PassesAllToService()
    {
        ...
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync("m1", "active", null, since, "tok", 25, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List("m1", "active", null, null, since, "tok", 25);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync("m1", "active", null, since, "tok", 25, null), Times.Once);
    }
    ...
    public async Task List_WithToolFilter_PassesToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, "claude", null, null, 50, null))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, null, "claude", null, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, "claude", null, null, 50, null), Times.Once);
    }

    // --- StatusGroup filter tests ---

    [Fact]
    public async Task List_WithStatusGroupLive_PassesToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 50, "live"))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, "live", null, null, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, null, null, 50, "live"), Times.Once);
    }

    [Fact]
    public async Task List_WithStatusGroupStale_PassesToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, null, null, null, null, 50, "stale"))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, null, "stale", null, null, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, null, null, null, null, 50, "stale"), Times.Once);
    }

    [Fact]
    public async Task List_WithStatusGroupAndStatus_PassesBothToService()
    {
        var pagedResult = new PagedResult<Session> { Items = [], ContinuationToken = null };
        _sessionService
            .Setup(s => s.ListAsync(null, "active", null, null, null, 50, "live"))
            .ReturnsAsync(pagedResult);

        var result = await _controller.List(null, "active", "live", null, null, null, 50);

        result.Should().BeOfType<OkObjectResult>();
        _sessionService.Verify(s => s.ListAsync(null, "active", null, null, null, 50, "live"), Times.Once);
    }
}
